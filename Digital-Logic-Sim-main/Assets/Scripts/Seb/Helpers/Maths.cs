...
namespace Seb.Helpers
{
	public static class Maths
	{
		...
		public static ulong TwosComplement(ulong unsignedValue, ulong numBits)
		{
			// Ensure numBits is valid (between 1 and 64)
			if (numBits < 1 || numBits > 64)
				return unsignedValue;
				
			// Create a mask for the specified number of bits
			ulong mask = (numBits == 64) ? ulong.MaxValue : ((1UL << (int)numBits) - 1);
			
			// Apply the mask to ensure we only consider the relevant bits
			unsignedValue &= mask;
			
			// Check if the most significant bit is set (negative number in two's complement)
			ulong msb = 1UL << ((int)numBits - 1);
			
			if ((unsignedValue & msb) != 0)
			{
				// If the MSB is set, this is a negative number in two's complement
				// For a negative number, we need to calculate its absolute value
				if (numBits == 64)
				{
					// Special case for 64 bits to avoid overflow
					return unsignedValue;
				}
				else
				{
					// Calculate the two's complement value
					// This is equivalent to (value - 2^numBits)
					return unsignedValue - (1UL << (int)numBits);
				}
			}
			
			// If MSB is not set, it's a positive number, return as is
			return unsignedValue;
		}
