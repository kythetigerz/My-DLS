...
namespace DLS.Game
{
	public class SubChipInstance : IMoveable
	{
		...
		// Add these properties and methods to SubChipInstance
		public string activationKeyString2 { get; private set; } = "";
		...
		// Add a new method to set the activation key as a string
		public void SetKeyChipActivationString(string keyString)
		{
			if (ChipType != ChipType.Key) throw new Exception("Expected KeyChip type, but instead got: " + ChipType);
			activationKeyString = keyString;

			// For backward compatibility, store the first character in InternalData
			// or a hash code if the string doesn't represent a single character
			if (keyString.Length == 1 && char.IsLetterOrDigit(keyString[0]))
			{
				InternalData[0] = keyString[0];
			}
			else
			{
				// Use a hash code for non-character keys
				InternalData[0] = (uint)(keyString.GetHashCode() & 0x7FFFFFFF);

				// Store the actual string somewhere for later retrieval
				// This could be in a static dictionary keyed by the hash
				SimKeyboardHelper.RegisterKeyString((int)InternalData[0], keyString);
			}
		}
