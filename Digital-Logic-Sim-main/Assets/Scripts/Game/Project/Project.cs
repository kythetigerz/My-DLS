...
namespace DLS.Game
{
	public class Project
	{
		...
		public void Update()
		{
			...
			{
				Simulator.UpdateInputFromMainThread();
			}
		...
		public void NotifyKeyChipBindingChanged(SubChipInstance keyChip, string keyBinding)
		{
			// Update the SubChipInstance
			keyChip.SetKeyChipActivationString(keyBinding);
			
			// Get the corresponding SimChip from the simulation
			SimChip simChip = rootSimChip.GetSubChipFromID(keyChip.ID);
			
			// Update the SimChip's internal state to match
			if (keyBinding.Length == 1 && char.IsLetterOrDigit(keyBinding[0]))
			{
				simChip.InternalState[0] = keyBinding[0];
			}
			else
			{
				// Use the same hash code calculation as in SubChipInstance
				simChip.InternalState[0] = (uint)(keyBinding.GetHashCode() & 0x7FFFFFFF);
				
				// Register the key string with the hash for later lookup
				SimKeyboardHelper.RegisterKeyString((int)simChip.InternalState[0], keyBinding);
			}			
		}
