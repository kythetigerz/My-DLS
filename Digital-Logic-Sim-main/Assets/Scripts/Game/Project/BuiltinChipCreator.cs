...
namespace DLS.Game
{
        public static class BuiltinChipCreator
        {
                ...
                static ChipDescription CreateStack_8()
                {
                        Color col = new(0.6f, 0.3f, 0.8f);

                        PinDescription[] inputPins =
                        {
                                CreatePinDescription("DATA", 0, PinBitCount.Bit8),
                                CreatePinDescription("PUSH", 1),
                                CreatePinDescription("POP", 2),
                                CreatePinDescription("CLOCK", 3)
                        };
                        PinDescription[] outputPins = { CreatePinDescription("OUT", 4, PinBitCount.Bit8) };
                        Vector2 size = new(GridSize * 10, SubChipInstance.MinChipHeightForPins(inputPins, outputPins));

                        return CreateBuiltinChipDescription(ChipType.Stack_8Bit, size, col, inputPins, outputPins);
                }
        ...
        static ChipDescription CreateKeyboardChip()
        {
                Color col = new(0.1f, 0.1f, 0.1f);
                Vector2 size = new Vector2(GridSize, GridSize) * 10;

                PinDescription[] outputPins = { CreatePinDescription("KEYS", 0, PinBitCount.Bit64) };

                return CreateBuiltinChipDescription(ChipType.Keyboard, size, col, null, outputPins);
        }
        
        static ChipDescription CreateMouseChip()
        {
                Color col = new(0.1f, 0.1f, 0.1f);
                Vector2 size = new Vector2(GridSize, GridSize) * 7;

                PinDescription[] outputPins = { CreatePinDescription("BUTTONS", 0, PinBitCount.Bit8) };

                return CreateBuiltinChipDescription(ChipType.Mouse, size, col, null, outputPins);
        }
