...
namespace DLS.Simulation
{
	public static class Simulator
	{
		...
		// A thread-local RNG so each worker has its own state:
		static readonly ThreadLocal<Random> tlRng = new(() => new Random());

		public static void StepChipParallel(SimChip chip)
		{
			// 1) Always do the inputs on this thread
			chip.Sim_PropagateInputs();

			// 2) Freeze / auto-freeze logic remains single-threaded
			if (FreezeChip.IsChipFrozen(chip))
				return;

			// ... (auto-freeze tracking code omitted for brevity) ...

			// 3) Gather your work items
			var subChips = chip.SubChips;
			int n = subChips.Length;

			// 4) Process all sub-chips in parallel
			//    We process built-ins in worker threads, and recurse on customs too.
			Parallel.For(0, n, index =>
			{
				var sub = subChips[index];

				// Optionally shuffle neighbors on rare frames
				if (canDynamicReorderThisFrame && index > 0 && !sub.Sim_IsReady()
					&& tlRng.Value.NextDouble() < 0.5)
				{
					// this swap is racy if two threads do it at once –
					// you could instead build a local reorder list first
					(subChips[index - 1], subChips[index]) = (subChips[index], subChips[index - 1]);
					sub = subChips[index];
				}

				// process it
				if (sub.IsBuiltin) ProcessBuiltinChip(sub);
				else StepChipParallel(sub);

				// then forward outputs
